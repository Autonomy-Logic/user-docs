...
Adding a Device is a three-step wizard. You'll work in two places: your browser (to create the Device) and a terminal on your Linux edge device (to install the agent).
...
The number of Devices you can pair at once is governed by your plan, see **[Plan limits](../../plans-and-billing/plan-limits)**.
...
|---|---|---|
| **Name** | Yes | Any human-readable name. Used to identify this Device everywhere in the platform. Examples: *Factory Floor Server*, *Lab Controller*, *Toradex Ivy*, *Demo*. |
| **Description** *(optional)* | No | What this Device is for. Helpful when you have several. |
...
> ```
>
> **c.** Once the installation finishes, the script will display a **Device ID**. Copy it, you'll paste it in the next step.
...
4. Display a **Device ID** in the terminal once it's running.

> The Device ID generated by the wizard **expires after 5 minutes**. If the install takes longer or you get distracted, just close the wizard and start step 1 again, there's no penalty for restarting.
...
- Marks the Device as paired.

A few seconds later the Device's status flips to **Active** (or **Connected**) on the Devices list. You're done.
...
After pairing, the Device card on the **[Devices list](devices-list)** shows live CPU, memory, and uptime metrics that match what `top` shows on the device. If those numbers stay at zero or the status stays **Inactive**, see **[Device not connecting](../../troubleshooting/device-not-connecting)**.

You can now:

- Add a vPLC → **[Creating a vPLC](../vplcs/creating-a-vplc)**.
- View detailed metrics → **[Device detail](device-detail)**.
...
This stops the agent and network-monitor containers, removes their images, and deletes the TLS material. The Device entry in the web app is not automatically deleted; remove it from **[Managing devices](managing-devices)**.
