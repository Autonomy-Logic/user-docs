...
Re-check the terminal on the edge device. The installer should have ended with a banner showing the **Device ID**. If it didn't, the install crashed mid-way.
...
Once the install finishes cleanly and prints a Device ID, return to the wizard and paste it.

## 2. Did the Device ID expire?
...
- `Unauthorized`: the Device was deleted in the cloud. Re-pair from the wizard, or run `curl https://getedge.me | bash` to regenerate.

## 5. Did somebody delete the Device in the cloud?
...
Fix: create a new Device in the wizard. Run the install command on the device again to get a fresh ID and pair the new Device.
...
- **Install or re-install the agent** → **[Installing the Device Agent](../platform/devices/installing-the-agent)**.
- **Manage Devices in the cloud** → **[Managing devices](../platform/devices/managing-devices)**.
- **vPLC stuck after the Device becomes Active** → **[vPLC stuck in Stopped](vplc-stuck-stopped)**.
